...
class AccountDatabase extends Dexie {
  accounts!: Table<AccountData, string>;

  constructor() {
    super('ViemAccountDB');
    this.version(1).stores({
      accounts: 'address, name', // 使用 address 作为主键
    });
  }
}
...
export const useAccountStore = create<AccountStoreState>((set, get) => ({
  ...
  createAccount: async (name, pin) => {
    ...
    const sessionKey = crypto.getRandomValues(new Uint8Array(32)).toString();
  ...
  deleteAccount: async (address) => {
    await db.accounts.delete(address);
    sessionManager.clearSession(address);
    tempEncryptedPrivateKey.delete(address);

    set((state) => {
      const newAccounts = state.accounts.filter((a) => a.address !== address);
      const newAccount =
  ...
  renameAccount: async (address, newName) => {
    ...
    await db.accounts.update(address, updatedAccount);
  ...
  addPasskeyAuth: async () => {
    ...
    await db.accounts.update(account.address, updatedAccount);
  ...
  setSessionDuration: (duration) => {
    ...
    // 更新数据库
    db.accounts.update(account.address, updatedAccount);
  ...
  _unlockPrivateKeyWithSessionKey: async (sessionKey) => {
    ...
    const encryptedPrivateKey = tempEncryptedPrivateKey.get(account.address);
...
if (typeof window !== 'undefined') {
  useAccountStore.getState()._loadAccounts();
}
